...
function doClick(e) {
	...
	for (i = 0; i < btnobjs.length; i++) {
		if (btnobjs[i] == button) {// 选的是这个按钮
			...
			btnobjs[i].style.backgroundImage = "url(/search/common/mytab/image/but-name05.gif)";
			btnobjs[i].select = "true";
		} else {
			btnobjs[i].className = "butind";
			btnobjs[i].style.backgroundImage = "url(/search/common/mytab/image/but-name04.gif)";
...
function initBody() {
	var temptourl = null;
	var btnobjs = document.all("tabbtn");
	for (i = 0; i < btnobjs.length; i++) {
