...
<script type="text/javascript" charset="utf-8">
	function show(flag, e) {
		if (flag == 0) //查看
		{
			...
			ShowModalWithResult('./courseManage/getCourseInforById.action?courseId='
					+ e.attributes('value').value + '&operateType=read','450px', '600px');
		}
		if (flag == 1) //选课
		{
			if (confirm("确认选修此课程？") == true) {
				window.location.href='./crManage/addCS.action?courseSelectObj.CourseId='
...
<body>
	...
	<ec:table items="courseInforList" var="record" listWidth="100%"
		action="./web/getRegisterCourseList.action" pageSizeList="10,15" rowsDisplayed="10"
		...
		<ec:row>
			...
			<ec:column width="15%" property="null" title="操作"
				...
				<a href='javascript:;' value='${record.COURSEID}'
					onclick='javascript:show(1,this);'>选课</a>
			</ec:column>
		</ec:row>
	</ec:table>
</body>
