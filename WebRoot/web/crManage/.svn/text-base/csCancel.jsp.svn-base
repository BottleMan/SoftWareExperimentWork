...
<script type="text/javascript" charset="utf-8">
	function show(e) {
		if (confirm("确认注销此课程？") == true) {
			ajaxRequest('web/crManage/deleteCS.action?courseId='
					+ e.attributes('value1').value + '&term='
					+ e.attributes('value2').value);
...
<body>
	...
	<ec:table items="csHisList" var="record" listWidth="100%"
		action="web/crManage/CSCancel.action?" pageSizeList="10,15"
		...
		<ec:row>
			...
			<ec:column width="25%" property="null" title="操作"
				...
				<a href='javascript:;' value1='${record.CourseId}'
					value2='${record.Term}' onclick='javascrip:show(this);'>注销课程</a>
			</ec:column>
		</ec:row>
	</ec:table>
</body>
